...
class SortingApp(QWidget):
    ...
    def sort_array(self):
        try:
            ...
            descriptions = {
                "Bubble Sort": "Элементы списка последовательно сравниваются друг с другом, и если два соседних элемента расположены в неправильном порядке, они меняются местами. ",
                "Selection Sort": "Опорный элемент делит массив на две части: меньшие элементы слева, большие справа, после чего процесс повторяется рекурсивно для каждой части.",
                "Insertion Sort": "Список разбивается на элементы, которые затем попарно сливаются в отсортированный итоговый список.",
                "Quick Sort": "Этот метод напоминает то, как люди сортируют карты в играх. На каждой итерации программа берет один элемент и вставляет его в нужное место в уже отсортированной части списка. ",
                "Merge Sort": "На каждой итерации из неотсортированной части выбирается самый маленький элемент и перемещается в конец отсортированной части. "
            }
